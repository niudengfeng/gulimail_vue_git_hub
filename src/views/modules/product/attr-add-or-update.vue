...
<!--      <el-form-item label="值类型" prop="valueType">
      ...
      <el-form-item label="所属分类" prop="catelogIdPath">
        <el-cascader
...
export default {
  ...
  watch: {
    catelogPath (catelogIdPathArray) {
      // 监听到路径变化需要查出这个三级分类的分组信息
      console.log('路径变了', catelogIdPathArray)
      this.attrGroups = []
      this.dataForm.attrGroupId = ''
      this.dataForm.catelogId = catelogIdPathArray[catelogIdPathArray.length - 1]
      this.dataForm.catelogIdPath =  catelogIdPathArray.join(",")
      if (catelogIdPathArray && catelogIdPathArray.length === 3) {
        this.$http({
          url: this.$http.adornUrl(
            `/product/attrgroup/list/${catelogIdPathArray[catelogIdPathArray.length - 1]}`
          ),
          method: 'get',
      ...
      } else if (catelogIdPathArray.length === 0) {
  ...
  methods: {
    changeWatch(){
      console.log("触发改变分类："+this.dataForm.catelogIdPathArray)
      this.catelogPath = this.dataForm.catelogIdPathArray
    },
    getTreeList () {
      this.$http({
        ...
        if (this.dataForm.attrId) {
          this.$http({
            ...
            if (data && data.code === 0) {
              ...
              let arr = data.attr.catelogIdPath.split(",");
              let numArr = arr.map((item, i) => {
                return parseInt(item);
              });
              this.dataForm.catelogIdPathArray = numArr
              this.dataForm.catelogIdPath = data.attr.catelogIdPath
              this.dataForm.showDesc = data.attr.showDesc
              this.catelogPath = numArr
              this.$nextTick(() => {
